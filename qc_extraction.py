...
def extract_errors_and_comments(doc):
    results = []
    # Updated shorthand map to be case-insensitive
    ...
    for table in doc.tables:
        if len(table.rows[0].cells) >= 4 and table.rows[0].cells[3].text.strip().lower() == "issue":
            for row in table.rows[1:]:
                ...
                # Find the error type in a case-insensitive way
                matched_type = next((et for et in error_types if et.lower() in first_line.lower()), first_line)

                # Use case-insensitive regex for matching category and severity
                ...
                if match:
                    ...
                    # Normalize the category to lower case before mapping
